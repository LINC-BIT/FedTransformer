...
def split_indices(num_cumsum, rand_perm):
    """根据总的样本data indexes和 分割规则 来确定每个client对应的 data indexes
    Args:
        num_cumsum: sections for splitting
        rand_perm: Data indexes of whole samples
    """
...
def homo_partition(client_sample_nums, num_samples):
    ...
    """
    rand_perm = np.random.permutation(num_samples)                  # shuffled data index of whole data samples
    num_cumsum = np.cumsum(client_sample_nums).astype(int)          # np.cumsum 数组累加
...
def hetero_dir_partition(targets, num_clients, num_classes, dir_alpha, min_require_size=None):
    ...
    while min_size < min_require_size:
        ...
        for k in range(num_classes):
            idx_k = np.where(targets == k)[0]       # 获取label中所有等于k值的索引： idx_k = [idx, idx...]
